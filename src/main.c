...
int main(void){
    ...
    // Włącz komendę RATE
    shell_rx_bytes(&sh, "rate 5\r\n"); // Wypisuj status co 5 ticków
    for(int i=0; i<12; i++) shell_tick(&sh);

    shell_rx_bytes(&sh, "echo hello world\r\n");
    for(int i=0;i<5;i++) shell_tick(&sh);
    
    // Test przepełnienia RX
    shell_rx_bytes(&sh, "echo BEFORE BURST\r\n");
    for(int i=0;i<5;i++) shell_tick(&sh);
    ...
    shell_rx_bytes(&sh, "echo AFTER BURST\r\n");
    for(int i=0;i<5;i++) shell_tick(&sh);

    // Wyłącz komendę RATE (rate=0)
    shell_rx_bytes(&sh, "rate 0\r\n");
